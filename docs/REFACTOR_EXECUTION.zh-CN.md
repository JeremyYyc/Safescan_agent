...
## 运行修正 — model-failure-handling

- 用户在根 env 更新 L1=`qwen3.7-flash`、L2/VL=`qwen3.7-plus-2026-05-26`、L3=`qwen3.8-max`。新配置在独立 Compose 容器中完成真实 API 探测：两个文本层、合成图片理解、L3 validate_report 工具请求均返回 200。旧运行容器仍使用原模型名，必须 recreate 才能加载新 env，单纯 restart 不更新容器环境。未改写用户 env 或提交密钥。
- 修复前数据库中的历史报告实际为 `qwen-max-latest` 的 404/model_not_found 错误对象。ReportWriter 现在对请求异常抛出受控异常，仅记录异常类型/HTTP 状态；JSON 格式错误仍进入原有有限修复流程。最终证据/保存节点、仓储写入和 HTTP complete 出口均禁止错误对象或无 regions 内容被当作成功报告。保留原三次修复及非空报告的既有校验政策，不改提示词或评分。
- PostgreSQL 的“已有报告”判断及列表标志仅认可 active 且含非空 regions 数组、无 error 的分析报告；前端历史加载同样排除错误记录，并防御伪 complete。PDF 历史单独保留原支持。不删除或修改历史记录，失败会话可再次发起分析。
- 本地相关后端测试 **50 passed / 1 skipped**；跳过的 PostgreSQL 检查在真实容器中用只读 CTE 执行通过（7 组内容/状态组合）；无业务数据写入。前端 3 项状态测试、lint/build 通过。真实历史失败会话的重试阻止标志已验证为 false。
- 后端复用上一版已验证依赖镜像，仅更新代码层，镜像内 **15 passed / 1 skipped**；前端标准 Docker 构建通过。PostgreSQL 最佳实践技能用于核对参数化查询和关联键，不引入 Supabase 服务或数据库迁移。本次模型探测仅使用极小合成输入，未重放用户视频的付费完整报告生成。
- 初始小额度探测通过后，进一步调用真实 L3 工具闭环被提供方以 **400 / Arrearage** 拒绝。此时停止所有付费探测；模型名有效不等于完整工作流可运行，仍需用户处理 API Key 所属账号的计费/余额/服务状态。统一模型客户端将该错误转成受控、可展示的提示；角色节点不再对该类失败重复发起业务重试，不保存错误报告。
- 用量边界：四次成功探测各设 max_tokens=128（只限制输出，并非总计费 Token）；另一次工具闭环最终失败，未记录各请求 usage，无法据本地记录精确核算费用。22:32 的用户视频分析在服务日志中有 9 次成功模型响应，非本次测试重放。截图的 24 模型/468 成功调用含本任务未调用的 GLM/DeepSeek/Kimi，不能全部归因于本项目或本次测试；需按账号、API Key、模型和时段对账。
