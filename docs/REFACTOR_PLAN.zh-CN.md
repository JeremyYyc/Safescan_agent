...
状态：仅规划；尚未开始业务代码重构。修订：按实现依赖重排五个目标，重要程度相同。本文替代此前分析报告中的生产化演进建议，作为本轮重构的范围依据。
...
- 当前工作仍在 refactor/plan；本轮仅修订并提交计划文档，不合并 main、不 push、不创建后续实现分支。计划验收后再按上述规则集成规划资料并创建 P1 分支。
...
完成 P1 后停止并交付；下一阶段才在 `refactor/mysql-to-postgresql` 替换数据库。
...
完成 P2 后停止；P3 使用本阶段确定的资源身份和仓储接口接入 MinIO。
...
完成 P4 后停止；P5 只集成已通过验收的工具，不重新定义 schema/存储方式。
