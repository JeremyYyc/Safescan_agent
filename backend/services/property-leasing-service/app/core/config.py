...
class Settings(BaseModel):
    ...
    identity_service_token_seconds: int = Field(default=300, ge=5, le=300)
    identity_token_refresh_skew_seconds: int = Field(default=30, ge=0, le=60)
    ...
    def from_env(cls) -> "Settings":
        return cls(
            ...
            identity_service_credential=SecretStr(
                os.getenv("PROPERTY_LEASING_IDENTITY_CREDENTIAL", "")
            ),
            ...
            identity_service_token_seconds=int(
                os.getenv("PROPERTY_LEASING_IDENTITY_TOKEN_SECONDS", "300")
            ),
            identity_token_refresh_skew_seconds=int(
                os.getenv("PROPERTY_LEASING_IDENTITY_TOKEN_REFRESH_SKEW_SECONDS", "30")
            ),
    ...
    def validate_runtime(self) -> None:
        ...
        if self.identity_token_refresh_skew_seconds >= self.identity_service_token_seconds:
            raise RuntimeError("Identity token refresh skew must be shorter than its lifetime")
        if (
            self.app_env == "production"
            and len(self.identity_service_credential.get_secret_value()) < 24
        ):
            raise RuntimeError(
                "PROPERTY_LEASING_IDENTITY_CREDENTIAL must be at least 24 chars in production"
            )
